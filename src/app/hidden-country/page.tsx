...
function nextSessionSeed(): string {
...
export default function GeodlePage() {
  ...
  useEffect(() => {
    ...
    // Don't reset if we just won or gave up - let the modal show with the game results
    if (isWon || didGiveUp) return

    // Try to restore game state from localStorage (for both daily completions and unlimited games)
    ...
    // Fallback: if returning to a completed daily and no progress cache exists, rebuild from saved daily result.
    if (dailyAlreadyCompleted && dailyResult && dailyResult.date === challengeDate) {
      const restoredTarget = countries.find((country) => country.name === dailyResult.targetName)
      if (restoredTarget) {
        const resultCodes = dailyResult.guessCodes ?? []
        const resultNames = dailyResult.guesses ?? []
        const restoredGuessCountries = resultCodes.length > 0
          ? resultCodes
              .map((code) => countries.find((country) => country.cca2 === code))
              .filter((country): country is Country => Boolean(country))
          : resultNames
        ...
        setTarget(restoredTarget)
        setGuesses(restoredGuessCountries.map((country) => createGeodleGuess(country, restoredTarget)))
        setDidGiveUp(dailyResult.outcome === 'give-up')
        setDidRecordResult(true)
        return
      }
    }

    // Only create a new game if not already completed daily
    if (dailyAlreadyCompleted && !isWon && !didGiveUp) return
    ...
    setDidGiveUp(false)
    setDidRecordResult(false)
  }, [challengeDate, countries, dailyAlreadyCompleted, dailyResult, didGiveUp, isWon, seedMode, seedOverride])
  ...
  useEffect(() => {
    ...
    // Only clean up progress for unlimited mode or when actively playing
    // For daily mode, keep the progress so it can be restored when returning to a completed puzzle
    if (!target) return
    if (seedMode === 'unlimited' && (isWon || didGiveUp)) {
      window.localStorage.removeItem(progressKey)
      return
    }
    // Don't save progress if daily already completed (keep the old progress for restoration)
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const progressKey = getProgressStorageKey(seedMode, challengeDate)
      // Only remove progress for unlimited mode; keep daily progress for restoration when returning
      if (seedMode === 'unlimited') {
        window.localStorage.removeItem(progressKey)
      }
    }
    ...
    if (seedMode === 'daily' && typeof window !== 'undefined') {
      ...
      const result: DailyGeodleResult = {
        ...
        guessCodes: guesses.length > 0 ? guesses.map((item) => item.guess.cca2) : (target?.cca2 ? [target.cca2] : []),
        outcome: 'win',
      }
  ...
  useEffect(() => {
    // Show completion modal if player just won/gave up, OR if returning to a completed daily game
    if (isWon || didGiveUp || (dailyAlreadyCompleted && (guesses.length > 0 || Boolean(dailyResult)))) {
      setShowCompletionModal(true)
    }
  }, [isWon, didGiveUp, dailyAlreadyCompleted, dailyResult, guesses.length])
  ...
  const playAgain = () => {
    ...
    const nextSeed = nextSessionSeed()
    setGuesses([])
    setTarget(null)
    setDidGiveUp(false)
    setDidRecordResult(false)
  ...
  const giveUp = () => {
    ...
    if (seedMode === 'daily' && typeof window !== 'undefined') {
      ...
      const result: DailyGeodleResult = {
        ...
        guessCodes: guesses.map((item) => item.guess.cca2),
        outcome: 'give-up',
      }
      window.localStorage.setItem(DAILY_RESULT_KEY, JSON.stringify(result))
      setDailyResult(result)
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <button
            ...
            onClick={() => {
              ...
              setGuesses([])
              setTarget(null)
              setSeedOverride(nextSessionSeed())
            }}
    ...
    >
      ...
      {showCompletionModal ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/45 px-4">
          <div className="gv-panel relative w-full max-w-[560px] p-5 text-center">
            <button
              type="button"
            ...
            {isWon ? (
              <>
                <p className="text-2xl font-bold text-[var(--accent-success)]">🎉 Solved!</p>
                <p className="mt-2 text-lg font-semibold text-[var(--text-primary)]">The country was {target?.name}</p>
                <p className="mt-3 text-sm text-[var(--text-muted)]">
                  You found it in <span className="font-semibold text-[var(--text-primary)]">{Math.max(1, guesses.length)} guess{guesses.length !== 1 ? 'es' : ''}</span>
                </p>
              </>
            ) : null}
            {didGiveUp ? (
              <>
                <p className="text-2xl font-bold text-[var(--accent-danger)]">Game Over</p>
                <p className="mt-2 text-lg font-semibold text-[var(--text-primary)]">The country was {target?.name}</p>
                <p className="mt-3 text-sm text-[var(--text-muted)]">
                  You made <span className="font-semibold text-[var(--text-primary)]">{guesses.length} guess{guesses.length !== 1 ? 'es' : ''}</span> before giving up
                </p>
              </>
            ) : null}
            {!isWon && !didGiveUp && dailyAlreadyCompleted && dailyResult ? (
              <>
                {dailyResult.outcome === 'win' ? (
                  <>
                    <p className="text-2xl font-bold text-[var(--accent-success)]">🎉 Solved!</p>
                    <p className="mt-2 text-lg font-semibold text-[var(--text-primary)]">The country was {target?.name ?? dailyResult.targetName}</p>
                    <p className="mt-3 text-sm text-[var(--text-muted)]">
                      You found it in <span className="font-semibold text-[var(--text-primary)]">{dailyResult.guessCount} guess{dailyResult.guessCount !== 1 ? 'es' : ''}</span>
                    </p>
                  </>
                ) : (
                  <>
                    <p className="text-2xl font-bold text-[var(--accent-danger)]">Game Over</p>
                    <p className="mt-2 text-lg font-semibold text-[var(--text-primary)]">The country was {dailyResult.targetName}</p>
                    <p className="mt-3 text-sm text-[var(--text-muted)]">
                      You made <span className="font-semibold text-[var(--text-primary)]">{dailyResult.guessCount} guess{dailyResult.guessCount !== 1 ? 'es' : ''}</span> before giving up
                    </p>
                  </>
                )}
              </>
            ...
            <div className="mt-4 flex flex-col gap-2">
              {seedMode === 'unlimited' ? (
                <button
                  ...
                  className="w-full rounded-md bg-[var(--accent)] px-4 py-2 font-semibold text-[var(--bg-base)]"
                >
                  Play again
                </button>
              ) : null}
              ...
              <button
                type="button"
                onClick={() => setShowCompletionModal(false)}
                className="gv-btn-outline px-4 py-2"
              >
                Close
              </button>
            </div>
