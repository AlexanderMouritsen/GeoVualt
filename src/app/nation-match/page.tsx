...
const WRONG_ANIMATION_MS = 420
const DAILY_LOCK_KEY = 'geovault-geoconnections-daily-completed'
const SOLVED_CARD_COLORS: SolvedCardColor[] = [
  { bg: '#f2cfc1', text: '#3a2318' },
  { bg: '#d7e7cc', text: '#24361c' },
  { bg: '#cfe1ef', text: '#1d2f40' },
  { bg: '#f2dfb8', text: '#3d2d16' },
]
...
export default function GeoConnectionsPage() {
  ...
  const isAnimatingWrong = wrongTileIds.length > 0
  ...
  useEffect(() => {
    ...
    setSuppressDailyLockModal(false)
  }, [seedMode, challengeDate])
  ...
  useEffect(() => {
    if (!dailyAlreadyCompleted || tiles.length === 0) return
    const allCodes = Array.from(new Set(tiles.map((tile) => tile.countryCode)))
    setSolvedCodes(allCodes)
    setSelected([])
    setMistakes(0)
  }, [dailyAlreadyCompleted, tiles])
  ...
  }, [challengeDate, dailyAlreadyCompleted, didRecordResult, isLost, isWon, recordResult, seedMode])

  const toggleTile = (tileId: string) => {
    if (dailyAlreadyCompleted || isWon || isLost || isAnimatingSolve || isAnimatingWrong) return
  ...
  const submitSelection = () => {
    if (dailyAlreadyCompleted || selected.length !== 4 || isWon || isLost || isAnimatingSolve || isAnimatingWrong) return
  ...
  return (
    ...
    >
            ...
            return (
              ...
              >
                <div className="flex items-center justify-center">
                  {first?.countryFlagUrl ? (
                    <Image
                      src={first.countryFlagUrl}
                      alt={`${first.countryName} flag`}
                      width={52}
                      height={34}
      ...
      {!isLoading && !error ? (
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {unsolvedTiles.map((tile) => {
            ...
            const isWrongTile = wrongTileIds.includes(tile.id)
            return (
              <button
                key={tile.id}
                type="button"
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={selected.length !== 4 || isWon || isLost || isAnimatingSolve || isAnimatingWrong}
      ...
      {showDailyLockModal ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/45 px-4">
          <div className="gv-panel relative w-full max-w-[520px] p-5 text-center">
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">You already completed today&apos;s Nation Links challenge.</p>
            <button
              type="button"
              onClick={() => {
              ...
              className="mt-4 w-full max-w-[460px] rounded-md bg-[var(--accent)] px-5 py-3 font-bold text-[var(--bg-base)]"
            >
              Switch to Unlimited
            </button>
          </div>
      ...
      <style jsx global>{`
        ...
        .gc-tile-wrong {
          animation: gc-wrong-shake ${WRONG_ANIMATION_MS}ms ease;
